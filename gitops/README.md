...
2. **Host-side / GitOps conflict on `placement` — real, but only on a
   *fresh* host; confirmed not to be firing on the current one.** I
   traced `openstack-network-plumbing.sh.j2`'s `helm upgrade placement
   ... -f ${OVERRIDES_DIR}/placement/2025.1-ubuntu_noble.yaml` and
   initially claimed this always errors ("no such file or directory").
   That was wrong for *this* host: `overrides/` was populated once by
   the original working script's Phase 6 fetch loop, and `helm-workspace`'s
   git-clone task uses `update: false` ("clone if missing, leave alone
   if present") — so on a host that already has
   `~/openstack-helm-workspace/openstack-helm` from that earlier manual
   run, Ansible never touches it and `overrides/` just persists.
   Confirmed working on the current host by directly checking service
   logs / the directory.

   The gap is narrower but still real: **nothing in this Ansible
   project would recreate `overrides/` on a genuinely fresh host** — a
   new instance, a rebuild after tearing this one down, anyone else
   cloning this repo and running `site.yml` cold. `values_overrides/`
   files live in the `openstack-helm` git repo itself, but the
   `overrides/` directory the plumbing script reads is generated by
   `helm osh get-values-overrides -d`, not shipped in git — a bare
   `git clone` (which is all `helm-workspace` does) won't produce it.
   On a fresh host this `helm upgrade` line would hit exactly the
   "no such file" failure I originally described, and — same mechanism
   as before — the script has no `set -e`, so it'd continue past that
   into a `kubectl delete job placement-db-init placement-db-sync`
   deleting Jobs that Argo CD (owning `placement` at wave 5) would then
   have to notice and recreate via `selfHeal`, on whatever its next
   reconcile happens to be, rather than promptly.

   Recommend the same fix as before, for the same reason: remove the
   `Triggering Placement Helm upgrade` / `Waiting for placement-db-sync`
   / `Restarting Placement API pods` / `Restarting Nova Conductor and
   Scheduler` block from `openstack-network-plumbing.sh.j2` and replace
   it with something Argo CD-native (e.g. a `PostSync` hook Job on
   `05-placement.yaml`). That closes the reproducibility gap directly,
   rather than patching it by adding a task that regenerates
   `overrides/` just to keep a script alive that a GitOps layer makes
   redundant anyway. The MariaDB tc.log/grastate cleanup and pod-recycle
   steps earlier in the same script don't touch `OVERRIDES_DIR` and are
   unaffected either way — those are correct as-is on any host, fresh
   or not.
